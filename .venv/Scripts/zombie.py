...
class Zombie:
    ...
    def build_behavior_tree(self):
        ...
        c1 = Condition("find boy", self.is_boy_nearby, 7)
        ...
        a5 = Action("get patrol loc", self.get_patrol_location)
        root = patrol = Sequence("get patrol loc", a5, a2)

        root = chase_or_pat = Selector("chase or pat", chase_boy, patrol)

        c2=Condition("check ball", self.zombie_win)
        a6 = Action("run away", self.run_away)
        root = catch_boy = Sequence("catch boy", c2, a4)
        root = run_or_chase = Selector("run or chase", catch_boy, a6)
        root = i_see_boy = Sequence("i see boy", c1, run_or_chase)
        root = find_or_pat = Selector("find or patrol", i_see_boy, patrol)
